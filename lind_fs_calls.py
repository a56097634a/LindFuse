...
Target: Modify the system call written in repy to satisfied new file system 
structure: 4k block file systems with free block lists and file metadata.

Fulfilled function: mount, cd, mkdir, rm, touch, and edit file, unmount and etc
...
superBlockName = 'linddata.0'

blockPrefix = 'linddata.'

superBlock = {}

rootBlockName = 'linddata.26'
...
def load_fs(name=superBlockName):
...
def _blank_fs_init():
  ...
  for i in range(0, 10000 + 1):
    createZeroNode(i)
 # for blockName in listfiles():
  #  if blockName.startswith(filePrefix):
   #   removefile(blockName)

  # Now setup superblock data structure
  ...
  for i in range(1, 26):
    freeBlocks[blockPrefix + str(i)] = freeBlockInit(i)
  ...
  rootBlock['mode'] = S_IFDIR | S_IRWXA
  rootBlock['atime'] = 1323630836
  rootBlock['ctime'] = 1323630836
  rootBlock['mtime'] = 1323630836
  rootBlock['linkcount'] = 4
  rootBlock['filename_to_inode_dict'] = {'d.':rootDirectoryInode, 'd..':rootDirectoryInode}
 
  persistSuperBlock(superBlockName)
  persistRootBlock(rootBlockName, rootBlock)


# These are used to initialize and stop the system
def persistSuperBlock(superBlockName):

  superBlockString = serializedata(superBlock)
  
  # open the file (clobber) and write out the information...
  
  superBlockFO = openfile(superBlockName,True)
  superBlockFO.writeat(superBlockString,0)
  superBlockFO.close()

def persistRootBlock(rootBlockName, rootBlock):

  rootBlockString = serializedata(rootBlock)

  # open the file (clobber) and write out the information...
  
  rootBlockFO = openfile(rootBlockName,True)
  rootBlockFO.writeat(rootBlockString,0)
  rootBlockFO.close()

def freeBlockInit(freeBlockNumber):
  freeBlockName = blockPrefix + str(freeBlockNumber)
  freeBlockArray = []
  if freeBlockNumber == 1:
    for i in range(27, 400):
      freeBlockArray.append(i)
  else:
    for i in range(((freeBlockNumber-1) * 400), (freeBlockNumber * 400)):
      freeBlockArray.append(i)
  
  freeBlockString = serializedata(freeBlockArray)
  
  freeBlockFO = openfile(freeBlockName,True)
  freeBlockFO.writeat(freeBlockString,0)
  freeBlockFO.close()

  return freeBlockArray
  
def restore_superblock(superBlockName):
  # should only be called with a fresh system...
  assert(superBlock == {})

  # open the file and write out the information...
  superBlockFo = openfile(superBlockName,True)
  superBlockString = superBlockFo.readat(None, 0)
  superBlockFo.close()

  # get the dict we want
  desiredSuperBlock = deserializedata(superBlockString)

  # I need to put things in the dict, but it's not a global...   so instead
  # add them one at a time.   It should be empty to start with
  for item in desiredSuperBlock:
    superBlock[item] = desiredSuperBlock[item]
  
  superBlock['mounted'] +=1
  print superBlock
  #read the freeblock into memory
  #assert(freeBlocks == {})
  _rebuild_freeblock()  



def _rebuild_freeblock():
  for i in range(1, 26):
    freeBlockArray = []
    freeBlockFO = openfile(blockPrefix + str(i), True)
    freeBlockString = freeBlockFO.readat(None, 0)
    freeBlockFO.close()
    print i
    freeBlockArray = deserializedata(freeBlockString)
    freeBlocks[blockPrefix + str(i)] = freeBlockArray
...
# create a file that contains all 0
def createZeroNode(node):
  fileName = blockPrefix + str(node)
  zeroFO = openfile(fileName, True)
  zeroString = '' * 4096
  zeroFO.writeat(zeroString, 0)
  zeroFO.close()
...
# get path list by absolute (parent) path
...
# get inode from parent node
def getNodeByParent(parentNode, nodeName):
  nextNode = 0
  #print 'parentNode = ', parentNode
  if parentNode != 0 and checkNodeType(parentNode) == 'd':
    parentBlockName = blockPrefix + str(parentNode)
    parentBlockFO = openfile(parentBlockName, True)
    parentNodeString = parentBlockFO.readat(None, 0)
    parentBlockFO.close()
    parentNodeData = deserializedata(parentNodeString)
    
    if parentNodeData['filename_to_inode_dict'].get('f'+ nodeName):
      nextNode = parentNodeData['filename_to_inode_dict'].get('f'+ nodeName)
    elif parentNodeData['filename_to_inode_dict'].get('d'+ nodeName):
      nextNode =  parentNodeData['filename_to_inode_dict'].get('d'+ nodeName)
    else:
      nextNode = 0
  #print 'nextnode = ', nextNode
  return nextNode

# find inode by path array
def findNodeByArray(pathArray):
  if pathArray[0] == '/':
    return 26
  nextNode = getNodeByParent(rootDirectoryInode, pathArray[0])
  pathArray = pathArray[1:]
  for item in pathArray:
    if nextNode != 0:
      nextNode = getNodeByParent(nextNode, item)
    else:
      nextNode = 0
      break
...
# check the type of Node By Node itself
def checkNodeType(node):
  blockName = blockPrefix + str(node)
  blockFO = openfile(blockName, True)
  blockString = blockFO.readat(None, 0)
  blockFO.close()
  blockData = deserializedata(blockString)
  if blockData.get('filename_to_inode_dict'):
    return 'd'
  else:
    return 'f'

# write node into parent dir block
def writeNodeToParent(parentNode, node, nodeName, nodeType):
  if parentNode != 0 and checkNodeType(parentNode) == 'd':
    parentNodeName = blockPrefix + str(parentNode)
    parentNodeFO = openfile(parentNodeName, True)
    parentNodeString = parentNodeFO.readat(None, 0)
    parentNodeData = deserializedata(parentNodeString)
    parentNodeData['linkcount'] += 1 
    if nodeType == 'd':
      parentNodeData["filename_to_inode_dict"]['d' + nodeName] = node
    else:
      parentNodeData["filename_to_inode_dict"]['f' + nodeName] = node
    

    parentNodeString = serializedata(parentNodeData)
    parentNodeFO.writeat(parentNodeString, 0)
    parentNodeFO.close()
    return True
  else:
    return False

# delete node from parent dir block
def deleteNodeFromParent(parentNode, node, nodeName, nodeType):
  if parentNode != 0 and checkNodeType(parentNode) == 'd':
    parentNodeName = blockPrefix + str(parentNode)
    parentNodeFO = openfile(parentNodeName, True)
    parentNodeString = parentNodeFO.readat(None, 0)
    parentNodeData = deserializedata(parentNodeString)
    parentNodeFO.close()
    removefile(parentNodeName)
    
    parentNodeData['linkcount'] = parentNodeData['linkcount'] - 1
    if nodeType == 'd':
      node = parentNodeData['filename_to_inode_dict'].pop('d' + nodeName)
    else:
      node = parentNodeData['filename_to_inode_dict'].pop('f' + nodeName)
...
# create dir node block
def createDirNode(parentNode, newNode):
  newNodeData = {
    'size': 0, 
    'uid': DEFAULT_UID, 
    'gid': DEFAULT_GID, 
    ...
    'mtime':1323630836,
    'linkcount':2,
    'filename_to_inode_dict': {'d.':newNode, 'd..':parentNode}}
  newNodeString = serializedata(newNodeData)
  newNodeName = blockPrefix + str(newNode)
  newNodeFO = openfile(newNodeName, True)
  newNodeFO.writeat(newNodeString, 0)
  newNodeFO.close()

  return newNode
  
# write data into index block
def writeIndexBlock(indexArray, data):
  position = 0
  for item in indexArray:
    if item == indexArray[-1]:
      removefile(blockPrefix + str(item))
      createZeroNode(item)
      dataFO = openfile(blockPrefix + str(item), True)
      write = data[position:(len(data) - 1)]
      dataFO.writeat(write, 0)
      dataFO.close()
    removefile(blockPrefix + str(item))
    createZeroNode(item)
    dataFO = openfile(blockPrefix + str(item), True)
    write = data[position:position + 4096]
    dataFO.writeat(write, 0)
    dataFO.close()
    position += 4096

# read data out of index block
def readIndexBlock(indexNode):
  indexFO = openfile(blockPrefix + str(indexNode), True)
  indexString = indexFO.readat(None, 0)
  indexArray = serializedata(indexString)
  indexFO.close()
...
# create index block
def createIndexBlock(indexNumber, fileNode):
  indexName = blockPrefix + str(fileNode)
  indexFO = openfile(indexName, True)
  indexString = indexFO.readat(None, 0)
  if indexString != '':
    indexArray = deserializedata(indexString)
  else:
    indexArray = []
  indexFO.close()
  if len(indexArray) != indexNumber:
    for item in indexArray:
      removefile(blockPrefix + str(item))
      createZeroNode(item)
      releaseFreeBlock(item)
    indexArray = []
    for i in range(0, indexNumber + 1):
      indexArray.append(requireFreeBlock())
    removefile(indexName)
    createZeroNode(fileNode)
    indexFO = openfile(indexName, True)
    indexString = serializedata(indexArray)
    indexFO.writeat(indexString, 0)
    indexFO.close()

  return indexArray

# create file node block
def createFileNode(newNode, size):
  indexNumber = size/4096 + 1
  indirect = 0
  fileNode = requireFreeBlock()
  if indexNumber > 1:
    indirect = 1
    createIndexBlock(indexNumber, fileNode)

  newNodeData = {'size':size, 'uid':1000, 'gid':1000, 'mode': 33261,'atime':1323630836, 'ctime':1323630836, 'mtime':1323630836,'linkcount':2, 'indirect':indirect, 'location':fileNode}
  newNodeString = serializedata(newNodeData)
  newNodeName = blockPrefix + str(newNode)
  newNodeFO = openfile(newNodeName, True)
  newNodeFO.writeat(newNodeString, 0)
...
# read node block into memory
def readNode(node):
  nodeName = blockPrefix + str(node)
  nodeFO = openfile(nodeName, True)
  nodeString = nodeFO.readat(None, 0)
  nodeData = deserializedata(nodeString)
  nodeFO.close()
  return nodeData

# read file node block into memory
def readFileNode(fileNode):
  fileNodeName = blockPrefix + str(fileNode)
  fileNodeFO = openfile(fileNodeName, True)
  fileNodeString = fileNodeFO.readat(None, 0)
  
  fileNodeData = deserializedata(fileNodeString)
  fileNodeFO.close()
  return fileNodeData

# read dir node block
def readDirNode(dirNode):
  dirNodeName = blockPrefix + str(dirNode)
  dirNodeFO = openfile(dirNodeName, True)
  dirNodeString = dirNodeFO.readat(None, 0)
  dirNodeData = deserializedata(dirNodeString)
  dirNodeFO.close()
  return dirNodeData

# change a free block file
def changeFreeBlock(freeBlockNumber, freeBlockArray):
  removefile(blockPrefix + str(freeBlockNumber))
  freeBlockName = blockPrefix + str(freeBlockNumber)
  freeBlockFO = openfile(freeBlockName, True)
  freeBlockString = serializedata(freeBlockArray)
  freeBlockFO.writeat(freeBlockString, 0)
...
# require a free block from free block arrays
def requireFreeBlock():
  blockNumber = 0
  while(blockNumber == 0):
    i = random.randint(1, 25)
    freeBlockArray = freeBlocks[blockPrefix + str(i)]
    if len(freeBlockArray) > 0:
      blockNumber = freeBlockArray[0]
      freeBlockArray = freeBlockArray[1:]
      freeBlocks[blockPrefix + str(i)] = freeBlockArray
      changeFreeBlock(i, freeBlockArray)

  return blockNumber

# release a block back into free block arrays
def releaseFreeBlock(blockNumber):
  freeBlockNumber = blockNumber/400 + 1
  freeBlocks[blockPrefix + str(freeBlockNumber)].append(blockNumber)
  freeBlockArray = freeBlocks[blockPrefix + str(freeBlockNumber)]
  changeFreeBlock(freeBlockNumber, freeBlockArray)

# count the number of free blocks
def getFreeBlockNumber():
  freeBlockNumber = 0
  for i in range(1, 26):
    freeBlockNumber += len(freeBlocks[blockPrefix + str(i)])
...
def statfs_syscall(path):
  ...
  try:
    ...
    pathArray = getPathArray(truepath)
    # is the path there?
    
    thisinode = findNodeByArray(pathArray)
    if thisinode == 0:
...
def access_syscall(path, amode):
 ...
 # """
  try:
    ...
    # success (return 0)
    fileBlock = readNode(thisinode)
    if fileBlock['mode'] & amode == amode:
      return 0
...
def chdir_syscall(path):
  ...
  pathArray = getPathArray(truepath)
  # If it doesn't exist...
  inode = findNodeByArray(pathArray)
  if inode == 0:
...
def mkdir_syscall(path, mode):
  ...
  try:
    ...
    truepath = _get_absolute_path(path)
    pathArray = getPathArray(truepath)
    newinodename = pathArray[-1]    
    
    thisinode = findNodeByArray(pathArray)
    #print 'mkdir', thisinode
    # is the path there?
    if thisinode != 0:
    ...
    parentPathArray = getPathArray(trueparentpath)
    parentinode = findNodeByArray(parentPathArray)
    
    #print 'mkdir', parentinode
    if parentinode == 0:
      raise SyscallError("mkdir_syscall","ENOENT","Path does not exist.")
    
    parenttype = checkNodeType(parentinode)
    ...
    #first, randomly allocate free block
    newinode = requireFreeBlock()
    
    print 'newinode', newinode
    # second, make the new directory...
    createDirNode(parentinode, newinode)    
    # ... and write it in the parent node..
    writeNodeToParent(parentinode, newinode, newinodename, 'd')
    #finally write entry in the node
...
def rmdir_syscall(path):
  ...
  try:
    ...
    # change the block file into all 0
    removefile(blockPrefix + str(thisinode))
    createZeroNode(thisinode)
    # We're ready to go!   Let's clean up the file entry
    dirname = pathArray[-1]
    print 'dirname, thisinode and parentinode', dirname, parentinode, thisinode
    # remove the entry from the parent...
    deleteNodeFromParent(parentinode, thisinode, dirname, 'd')
...
def link_syscall(oldpath, newpath):
  ...
  try:
    ...
    oldPathArray = getPathArray(trueoldpath)
    oldinode = findNodeByArray(oldPathArray)
    # is the old path there?
    if oldinode == 0:
      raise SyscallError("link_syscall","ENOENT","Old path does not exist.")

    nodeType = checkNodeType(oldinode)
    ...
    newPathArray = getPathArray(truenewpath)
    newinode = findNodeByArray(newPathArray)
    ...
    newParentPathArray = getPathArray(truenewparentpath)
    newParentNode = findNodeByArray(newParentPathArray)
    
    if newParentNode == 0:
      raise SyscallError("link_syscall","ENOENT","New path does not exist.")

    parentNodeType = checkNodeType(newParentNode)
    ...
    newfilename = newPathArray[-1]
    # first, add node entry to new parent
    t = 'f'
    writeNodeToParent(newParentNode, oldinode, newfilename, t)
    # ... and the file itself
    fileBlock = readFileNode(oldinode)
    fileBlock['linkcount'] += 1
    fileBlockString = serializedata(fileBlock)
    fileBlockFO = openfile(blockPrefix + str(oldinode), True)
    fileBlockFO.writeat(fileBlockString, 0)
    fileBlockFO.close()
...
def unlink_syscall(path):
  ...
  try:
    ...
    # remove the entry from the parent...
    deleteNodeFromParent(parentinode, thisinode, filename, 'f')

    # 
    fileBlock = readFileNode(thisinode)
    fileNode = fileBlock['location']
    print 'fileNode: ', fileNode
    fileBlock['linkcount'] -= 1
    indirect = fileBlock['indirect']
    if fileBlock['linkcount'] == 1 and indirect == 0:
      removefile(blockPrefix + str(thisinode))
      createZeroNode(thisinode)
      removefile(blockPrefix + str(fileNode))
      createZeroNode(fileNode)
      releaseFreeBlock(thisinode)
      releaseFreeBlock(fileNode)
    elif indirect == 1 and fileBlock['linkcount'] == 1:
      fileFO = openfile(blockPrefix + str(fileNode), True)
      fileString = fileFO.readat(None, 0)
      fileNodes = deserializedata(fileString)
      print fileNodes
      fileFO.close()
      ...
      fileBlockString = serializedata(fileBlock)
      fileBlockFO = openfile(blockPrefix + str(thisinode), False)
      fileBlockFO.writeat(fileBlockString, 0)
      fileBlockFO.close()
...
def open_syscall(path, flags, mode):
  ...
  try:
    ...
    if thisinode == 0:
      ...
      trueparentpath = _get_absolute_parent_path(path)
      parentPathArray = getPathArray(trueparentpath)
      parentNode = findNodeByArray(parentPathArray)
      print 'parentnode: ', parentNode
      if parentNode == 0:
        raise SyscallError("open_syscall","ENOENT","Path does not exist.")
        
      parentNodeType = checkNodeType(parentNode)
      if parentNodeType != 'd':
      ...
      fileNode = createFileNode(newinode, 0)
      nodeType = checkNodeType(newinode)
      nodeBlock = readNode(newinode)
      writeNodeToParent(parentNode, newinode, filename, 'f')
      
      thisinode = newinode
      
      # this file must not exist or it's an internal error!!!
      #openfile(blockPrefix + str(fileNode),True).close()
    ...
    # if the file did exist, were we told to create with exclusion?
      ...
      nodeType = checkNodeType(thisinode)
      nodeBlock = readNode(thisinode)
      #fileNode = nodeBlock['location']
      # This file should be removed.   If O_RDONLY is set, the behavior
      # is undefined, so this is okay, I guess...
      if O_TRUNC & flags:
        #nodeBlock = readFileNode(thisinode)
        fileNode = nodeBlock['location']
        #inode = fastinodelookuptable[truepath]
        # if it exists, close the existing file object so I can remove it...
        if fileNode in fileobjecttable:
          fileobjecttable[fileNode].close()
          # reset the size to 0
        print 'trunc'
        nodeBlock['size'] = 0
        nodeBlockString = serializedata(nodeBlock)
        nodeBlockFO = openfile(blockPrefix + str(thisinode), True)
        nodeBlockFO.writeat(nodeBlockString, 0)
        nodeBlockFO.close()
        # remove the file...
        removefile(blockPrefix + str(fileNode))

        # always open the file.
        fileobjecttable[fileNode] = openfile(blockPrefix + str(fileNode),True)
    ...
    if nodeType == 'f':
      fileNode = nodeBlock['location']
      if fileNode not in fileobjecttable:
        thisfo = openfile(blockPrefix + str(fileNode),False)
        fileobjecttable[fileNode] = thisfo
    # I'm going to assume that if you use O_APPEND I only need to 
    # start the pointer in the right place.
    if O_APPEND & flags:
      print 'append'
      position = nodeBlock['size']
    else:
    ...
    filedescriptortable[thisfd] = {'position':position, 'inode':thisinode, 'lock':createlock(), 'flags':flags&O_RDWRFLAGS}
...
def lseek_syscall(fd, offset, whence):
  ...
  try:
    ...
    nodeBlock = readNode(inode) 
    fileSize = nodeBlock['size']
...
def read_syscall(fd, count):
  ...
  try:
    ...
    indirect = readFileNode(inode)['indirect']
    fileNode = readFileNode(inode)['location']
    if indirect == 0:
      data = fileobjecttable[fileNode].readat(count,position)
    else:
      fileString = fileobjecttable[fileNode].readat(None, 0)
      fileNodes = deserializedata(fileString)
      print 'file blocks', fileNodes
      data = ''
      for item in fileNodes:
        fileFO = openfile(blockPrefix + str(item), True)
        dataString = fileFO.readat(None, 0)
        data = data + dataString 
        fileFO.close()
      data = data[position:count+position]
...
def write_syscall(fd, data):
  ...
  try:
    ...
    if blankbytecount > 0:
      ...
      blank = '\0' * blankbytecount
      data = blank + data
      position = fileSize
      if indirect == 0 and length < 4096:
        fileobjecttable[fileNode].writeat('\0'*blankbytecount,fileSize)
    else:
      if (len(data) + position) > fileSize:
    ...
    odata = ''
    if indirect == 0 and length < 4096:
      fileobjecttable[fileNode].writeat(data,position)
    else:
      fileBlock['indirect'] = 1

      if fileSize < 4096:
        odata = fileobjecttable[fileNode].readat(None, 0)
        fileobjecttable[fileNode].close()
        removefile(blockPrefix + str(fileNode))
        createZeroNode(fileNode)
      else:
        ...
        for item in fileNodes:
          fileFO = openfile(blockPrefix + str(item), True)
          dataString = fileFO.readat(None, 0)
          odata = odata + dataString
          fileFO.close()
        fileobjecttable[fileNode].close()

      indexNumber = length/4096 + 1
      if fileSize == 0 or position == fileSize:
        odata = odata + data
      elif length == fileSize:
        odata.replace(odata[position:position+len(data)], data, 1)
      else:
        if position < fileSize and (position + len(data)) >= fileSize:
          odata.replace(odata[position:fileSize], data, 1)

      indexArray = createIndexBlock(indexNumber, fileNode)
      print 'fileNodes', indexArray
      writeIndexBlock(indexArray, odata)
      fileobjecttable[fileNode] = openfile(blockPrefix + str(fileNode), True)
    ...
    if filedescriptortable[fd]['position'] > fileSize:
      fileBlock['size'] = filedescriptortable[fd]['position']

    fileBlockFO = openfile(blockPrefix + str(inode), True)
    fileBlockString = serializedata(fileBlock)
    fileBlockFO.writeat(fileBlockString, 0)
    fileBlockFO.close()
...
def getdents_syscall(fd, quantity):
  ...
  try:
    ...
    filedescriptortable[fd]['position'] = min(startposition + len(returninodefntuplelist),\
      len(dirBlock['filename_to_inode_dict']))
...
def ftruncate_syscall(fd, new_len):
  ...
  try: 
    ...
    if blankbytecount > 0:
      # let's write the blank part at the end of the file...
      raise("ftruncate_syscall", "EINVAL", "Can not use truncate to add length")
    
    odata = ''
    if indirect == 0 and fileSize <= 4096 and new_len <= 4096:
      indirect = 0
      # we must cut
      to_save = fileobjecttable[fileNode].readat(new_len,0)
      fileobjecttable[fileNode].close()
      # remove the old file
      removefile(blockPrefix + str(fileNode))
      # make a new blank one
      fileobjecttable[fileNode] = openfile(blockPrefix+str(fileNode),True)
      fileobjecttable[fileNode].writeat(to_save, 0)

    elif indirect == 1 and fileSize > 4096 and new_len > 4096:
      indirect = 1
      fileString = fileobjecttable[fileNode].readat(None, 0)
      fileNodes = deserializedata(fileString)
      print 'file blocks', fileNodes
      ...
      indexNumber = new_len/4096 + 1
      odata = odata[0:new_len]
      indexArray = createIndexBlock(indexNumber, fileNode)
      print 'fileNodes', indexArray
      writeIndexBlock(indexArray, odata)
      fileobjecttable[fileNode] = openfile(blockPrefix + str(fileNode), True)
      ...
      removefile(blockPrefix + str(fileNode))
      fileobjecttable[fileNode] = openfile(blockPrefix + str(fileNode), True)
      fileobjecttable[fileNode].writeat(odata, 0)
    
    fileBlock['indirect'] = indirect
    fileBlock['size'] = new_len
    removefile(blockPrefix + str(inode))
    fileBlockFO = openfile(blockPrefix + str(inode), True)
    fileBlockString = serializedata(fileBlock)
    fileBlockFO.writeat(fileBlockString, 0)
    fileBlockFO.close()
...
def rename_syscall(old, new):
  ...
  try:
    ...
    pathArray = getPathArray(true_new_path)
    newNode = findNodeByArray(pathArray)
    pathArray = getPathArray(true_old_path)
    oldNode = findNodeByArray(pathArray)
    if oldNode == 0:
      raise SyscallError("rename_syscall", "ENOENT", "Old file does not exist")

    if newNode != 0:
      raise SyscallError("rename_syscall", "ENOENT", "New file alread exists")
    
      

    trueparentpath_old = _get_absolute_parent_path(true_old_path)
    pathArray = getPathArray(trueparentpath_old)
    oldParentNode = findNodeByArray(pathArray)
    
    trueparentpath_new = _get_absolute_parent_path(true_new_path)
    pathArray = getPathArray(trueparentpath_new)
    newParentNode = findNodeByArray(pathArray)
    
    newname = newPathArray[-1]
    oldname = oldPathArray[-1]

    nodeType = checkNodeType(oldNode)
    # change the 'd..' if the old node is a directory
    if nodeType == 'd':
      dirBlock = readDirBlock(oldNode)
      dirBlock['filename_to_inode_dict']['d..'] = newParentNode
      dirString = serializedata(dirBlock)
      dirFO = openfile(blockPrefix + str(oldNode))
      dirFO.writeat(dirString, 0)
    
    #delete from the old parent
    deleteNodeFromParent(oldNode, oldParentNode, oldname, nodeType)
    
    # add node to the new parent
    parentNodeType = checkNodeType(newParentNode)
    nodeType = checkNodeType(newNode)
    # check to see if the new parent is a directory
    if parentNodeType != 'd':
      raise SyscallError("mkdir_syscall","ENOTDIR","Path's parent is not a directory.")
    
    # write node into new parent and return success
    writeNodeToParent(newParentNode, newNode, newname, nodeType)
    return 0
  finally:
    pass
